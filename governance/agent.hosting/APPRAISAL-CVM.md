...
## Constitution compatibility (reconciled 2026-09-18)

The reconciled Steward source accepts `azure_cvm` with VMPL 0..3 and nonempty
AK issuer/root allowlists, and accepts attested `SVCB` grants. These changes
are source compatibility only; the deployed constitution must be upgraded
through governance before use. The candidates still have empty AK roots and
remain blocked. Compose the constitution through the normal Steward source
workflow; source reconciliation is not a live constitution update.
